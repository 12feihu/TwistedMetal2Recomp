...
#
# Writes: dist/twistedmetal2-<VERSION>-<artifact-tag>.zip
set -euo pipefail
...
exec bash "${PACKAGER}" \
  ...
  --zip-prefix twistedmetal2 \
  --exe-name TwistedMetal2_Recompiled \
  --display-name "TwistedMetal2 Recompiled" \
  --recompiler-build "${RECOMPILER_BUILD}" \
  --version-env RELEASE_VERSION \
  --disc-hint "your legally owned TwistedMetal2 disc" \
